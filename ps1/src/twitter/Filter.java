...
public class Filter {
    ...
    public static List<Tweet> inTimespan(List<Tweet> tweets, Timespan timespan) {
        ...
        for (final Tweet t : tweets) {
            final Instant time = t.getTimestamp();
    ...
    public static List<Tweet> containing(List<Tweet> tweets, List<String> words) {
        ...
        for (Tweet tweet : tweets) {
            final String[] wordListFromTweet = tweet.getText().split(regex);
            for (String wordFromTweet : wordListFromTweet) {
                if (result.contains(tweet)) {
                    break;
                }
                for (String word : words) {
                    if (word.toLowerCase().equals(wordFromTweet.toLowerCase())) {
                        result.add(tweet);
                        break;
                    }
                }
